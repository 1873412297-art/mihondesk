...
2026-09-18 便携子项进展：已实现配置锁、完整旧程序与便携数据保留、更新启动保护、持久化恢复记录及 `-Recover`；两次目录重命名之间的缺口由恢复记录处理，不宣称单次原子交换。PowerShell 7/5.1 各 9 个场景通过，真实 EXE 验证了合成旧结构配置升级成功和迁移失败后原目录逐文件恢复。已构建并核验本地 0.2.18 便携 ZIP，见[便携升级与回滚证据](../evidence/2026-09-18-portable-update-recovery.md)。历史 0.1.3 安装包、安装版/外部配置目录回滚、文件关联及卸载仍未完成；应用更新服务尚无界面入口。
