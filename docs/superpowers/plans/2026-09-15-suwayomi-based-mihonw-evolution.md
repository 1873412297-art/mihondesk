...
2026-09-18 子项进展：自动备份调度、原子写入失败保护、路径校验及损坏文件拒绝已验证；桌面恢复已增加进度和可回滚取消，并通过 10,000 漫画 / 40,000 章节用例及应用镜像验证。数据库结构升级前已增加包含 WAL 数据的独立快照、完整性校验、失败保护及恢复验证；真实 EXE 的 7 个隔离升级场景通过。见 [备份可靠性](../evidence/2026-09-18-backup-reliability.md)、[恢复进度与取消](../evidence/2026-09-18-backup-restore-progress.md) 和 [迁移前数据库快照](../evidence/2026-09-18-pre-migration-snapshot.md)。完整配置目录升级回滚与真实跨应用备份样本尚未完成，因此本整项保持未勾选。
...
2026-09-18 详情子项：修复在线详情请求脱离页面后继续运行的问题；加载页增加返回/Esc，离开页面取消请求，延迟响应不再重新选中旧详情或继续写入。实际 BrowseContentView/同步服务/SQLite 回归与现有详情操作共 34 项源码、34 项包内测试通过；见[详情加载取消证据](../evidence/2026-09-18-detail-loading-cancellation.md)。本项其余阅读器与 host 恢复场景仍需完整验收。

2026-09-18 浏览子项：修复追加分页失败后重试上一页并覆盖已有列表、搜索草稿混入下一页请求的问题。现在重试失败页并保留累计结果，翻页沿用已提交关键词，新搜索才重置列表；源码和包内各 21 项相关回归通过，见[图源分页恢复证据](../evidence/2026-09-18-source-pagination-retry.md)。

2026-09-18 筛选子项：弹窗改用独立草稿，取消与重置不再提前修改正在使用的筛选；应用时仅回写状态并保留图源自定义筛选类，同时修复勾选状态未即时重绘。含分组默认值、自定义子类与后续翻页的源码/包内各 23 项回归通过，见[筛选草稿隔离证据](../evidence/2026-09-18-filter-draft-isolation.md)。
...
2026-09-18 长测子项：当前 0.2.18 实际 EXE 在同一 JVM 连续完成 1800.684 秒、520 轮、19,760 次解码，退出 0 且无 stderr；核心预算高水位 163.860 MiB。原始堆/进程采样与图表已归档。但最后五分钟进程私有内存中位数升至 1119.275 MiB，尚不能确认稳定平台，需进一步定位分配与回收；本轮为 1–8 页合成章节的无界面负载，不覆盖长章节和 Compose 帧率，见[当前阅读长测证据](../evidence/2026-09-18-current-reader-soak.md)。
...
**修改：** `desktop-app/build.gradle.kts`、`desktop-app/src/main/kotlin/mihon/desktop/updates/DesktopAppUpdateService.kt`、`scripts/mihondesk-updater.ps1`、`scripts/verify-desktop-clean-machine.ps1`、`README.md`。
...
2026-09-18 便携子项进展：已实现配置锁、完整旧程序与便携数据保留、更新启动保护、持久化恢复记录及 `-Recover`；两次目录重命名之间的缺口由恢复记录处理，不宣称单次原子交换。PowerShell 7/5.1 各 9 个场景通过，真实 EXE 验证了合成旧结构配置升级成功和迁移失败后原目录逐文件恢复。已构建并核验本地 0.2.18 便携 ZIP，见[便携升级与回滚证据](../evidence/2026-09-18-portable-update-recovery.md)。历史 0.1.3 安装包、安装版/外部配置目录回滚、文件关联及卸载仍未完成。

2026-09-18 应用更新入口进展：“关于”已接入主动检查、版本说明、按发行模式选择附件、带进度且可取消的强制校验下载，以及文件夹/发布页面入口。70 项源码和 71 项包内测试通过，真实 GitHub 清单及新 EXE 启动通过；见[应用内更新证据](../evidence/2026-09-18-in-app-update-download.md)。随后已接通默认配置便携版的“退出并更新”、准备握手、完整启动器进程链退出等待、结果日志与重启；最终 88 项包内测试通过（1 项联网测试跳过），两种 PowerShell 各 16 场景通过，并完成隔离真实 GUI 退出、替换、失败回滚重启与配置保留验证；同时修复 Windows 下载目标临时占用导致的保存失败，验证占用释放、持续占用和取消三种情况；见[便携更新交接证据](../evidence/2026-09-18-portable-update-handoff.md)。安装版自动交接与回滚仍未完成；T11 继续保持未完成。
